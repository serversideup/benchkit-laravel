<template>
    ...
    <div class="flex-1 min-h-0 w-full flex flex-col px-8 pt-6 pb-8">
        <div class="flex items-center justify-between mb-5 shrink-0">
            ...
            <button v-if="state === 'running'" @click="cancelQueue()" type="button" class="px-4 py-2.5 rounded-lg text-sm font-medium text-[#CECFD2] bg-[#0C0E12] border border-[#373A41] hover:bg-[#13161B] hover:border-[#61656C] transition-colors duration-200 cursor-pointer">Cancel run</button>
        ...
        <div class="flex-1 min-h-0 flex gap-6">
            ...
            <div class="flex-1 min-h-0 flex flex-col">
                <div v-if="state === 'running' && activeBenchmark === 'yabs' && form.geekbench" class="flex items-center rounded-xl bg-[#13161B] p-4 border border-[#373A41] font-mono text-sm text-[#CECFD2] shrink-0 mb-4">
                ...
                <div v-if="state === 'running' && activeBenchmark === 'php' && form.php_mode === 'full'" class="flex items-center rounded-xl bg-[#13161B] p-4 border border-[#373A41] font-mono text-sm text-[#CECFD2] shrink-0 mb-4">
                    <img src="/images/icons/warning.svg" alt="Warning" class="h-5 w-5 mr-2">
