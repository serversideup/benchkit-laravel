<template>
    <div class="flex-1 w-full flex flex-col">
        ...
        <div v-else-if="state == 'completed' && saveState === 'empty'" key="empty" class="state-in flex-1 flex flex-col items-center justify-center gap-4">
            ...
            <button @click="backToHome()" type="button" class="px-4 py-2.5 rounded-lg text-sm font-medium text-[#CECFD2] bg-[#0C0E12] border border-[#373A41] hover:bg-[#13161B] hover:border-[#61656C] transition-colors duration-200 cursor-pointer">
                Back to home
            </button>
        </div>
...
// over from in-memory state. The queue only resets AFTER the visit finishes —
// resetting first would flash the Home page mid-navigation. Immediate: the
// run may have finished (and saved) while this watcher was unmounted.
