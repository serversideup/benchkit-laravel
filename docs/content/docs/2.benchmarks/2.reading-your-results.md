...
This is not hidden — it is drawn. The `/bench/io` curve on your result climbs in a straight line and then goes flat, and the line it flattens onto is `workers ÷ delay`, worked out from arithmetic before the test ran. Where it bends is your worker count. The shipped pool size is fixed rather than sized to your hardware, so on a large machine that bend arrives early, and [Common tuning strategies](/docs/configuration/common-tuning-strategies) covers what to do about it.
...
That is not a small effect. A server answering in a fraction of a millisecond behind a 13ms round trip spends 98% of every connection's life in transit — reaching the same pool from there takes roughly fifty times the connections it would from next door. BenchKit measures the round trip at the handshake, sizes the sweep accordingly, and says so on the result when even that was not enough. Use a machine in the same datacenter; a laptop across the internet answers a different question.
...
- the application was set up the way you would deploy it — cached config and routes, OPcache on, debug off
