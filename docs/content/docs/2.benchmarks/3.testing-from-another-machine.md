...
How many connections there can be is a property of the generator machine, not a setting: each one is an open file and an ephemeral port, and the pairing command measures both limits, raises the file limit as far as it may without privileges, and reports what is left. The dialog turns that into the most it could measure from where it is. Your server is not slower. You cannot ask it for more from that far away.

BenchKit measures the round trip when the generator connects and sizes the sweep from it, so a generator further away is asked for proportionally more connections, up to what it can hold open. When even that was not enough, the result says so — that is your cue to move the generator closer, or to raise its open file limit, not to distrust the server.
::
...
::warning
Under PHP-FPM a request holds a worker for its whole duration. If your connection count is above `pm.max_children`, you are measuring the pool rather than the server. Raise `PHP_FPM_PM_MAX_CHILDREN` first, sized from memory rather than cores. See [Common tuning strategies](/docs/configuration/common-tuning-strategies#the-worker-pool).
::
