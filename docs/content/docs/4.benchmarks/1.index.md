...
::note
The four load test routes are open endpoints, so you can also probe them ad hoc with your own tools — including the corrected-latency method that measures tail latency honestly. See [Testing from your own machine](/docs/benchmarks/testing-from-your-own-machine). For results recorded into the run, use the external load test mode instead.
::
