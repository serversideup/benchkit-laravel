...
BenchKit is not a tuned server. It turns OPcache on, caches configuration, routes and events, and otherwise leaves the [serversideup/php](https://serversideup.net/open-source/docker-php) defaults where they are.
...
::note
Everything on this page is an environment variable. Change it, restart the container, and run again. Nothing here needs a rebuild. The blocks below are fragments. Drop them into the `environment:` section of a complete file, like the one in [Customizing the image](/docs/configuration/customizing-the-image).
::
...
You can see the ceiling on your own result. The `/bench/io` curve climbs in a straight line and then goes flat, and the height of the flat part is drawn from arithmetic: `workers ÷ delay`. Read the height rather than the bend — a connection only occupies a worker while the server holds the request, so from a distant generator it takes several connections to keep one worker busy and the bend lands well right of your pool size.

The image ships with `ondemand` and a pool of 20 workers, which is the serversideup/php default for the FPM variations. These are the variables that change it:
...
The observation: on a four-core box, response times were fine when idle and poor once busy. A single request came back in 13 ms, and under load the slowest 5% took an order of magnitude longer. That is the shape of a queue, and one likely cause is a pool that starts workers on demand — every time traffic outruns the few spare workers, PHP-FPM forks a new one and boots the application before it can answer anything.
...
| | `ondemand` | `static` |
| --- | ---: | ---: |
| Median response time | 26 ms | **83 ms** |
...
BenchKit uses SQLite unless you point it elsewhere. SQLite runs inside the PHP process, so it has no connection cost and none of the contention behavior of a database server — which makes it a poor stand-in if your application talks to MySQL or Postgres.
