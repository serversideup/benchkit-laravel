...
::tip{to="/docs/configuration/common-tuning-strategies"}
For which settings are worth changing first, and a worked example of a change that made things worse, see [Common tuning strategies](/docs/configuration/common-tuning-strategies).
::
...
::tabs{sync="php-ini"}
  :::tabs-item{label="Mount a file" icon="i-lucide-file-cog"}
    ...
    ; conf.d loads alphabetically, so the zzz prefix puts this after the file
    ; serversideup/php writes from your environment variables. Anything set
    ; here wins where the two overlap.

    ; Neither setting below has an environment variable. Both are raised from
    ; the PHP defaults of 4096K and 5.
