---
title: Adding a real database
description: Point BenchKit at MySQL, MariaDB, or Postgres instead of the default SQLite.
---

BenchKit uses SQLite by default so that it runs with no setup at all. SQLite runs inside the PHP process, which means it has no connection cost and none of the indexing and contention behavior you get from a database server. If you want numbers that reflect how your application talks to a real database, use Laravel's native database drivers to point BenchKit at MySQL, MariaDB, or Postgres.

## The two files

Put both files in the same directory. Compose reads `.env` on its own, so you write each value once and it reaches both containers:

- **`.env`** holds the credentials.
- **`docker-compose.yml`** hands them to BenchKit as Laravel's `DB_*` variables, and to the database as the variables its own image expects.

Pick your engine and copy both files.

::tabs{sync="database"}
  :::tabs-item{label="MySQL" icon="i-simple-icons-mysql"}
    ::::code-tree{defaultValue=".env"}
    ```ini [.env]
    # BenchKit reads these as Laravel's database config.
    # DB_HOST is the service name below, not localhost.
    DB_CONNECTION=mysql
    DB_HOST=mysql
    DB_PORT=3306
    DB_DATABASE=benchkit
    DB_USERNAME=benchkit
    DB_PASSWORD=benchkit_secret

    # Only the database container uses this one.
    DB_ROOT_PASSWORD=root_secret
    ```

    ```yaml [docker-compose.yml]
    services:
      benchkit:
        image: serversideup/benchkit-laravel:latest
        ports:
          - "80:8080"
        environment:
          DB_CONNECTION: ${DB_CONNECTION}
          DB_HOST: ${DB_HOST}
          DB_PORT: ${DB_PORT}
          DB_DATABASE: ${DB_DATABASE}
          DB_USERNAME: ${DB_USERNAME}
          DB_PASSWORD: ${DB_PASSWORD}
        volumes:
          - benchkit-runs:/var/www/html/storage/app/runs
        depends_on:
          mysql:
            condition: service_healthy

      mysql:
        image: mysql:8.4
        # Must stay above the app's PHP worker count: each worker holds its own
        # connection, and the default of 151 is below what a large host runs.
        command: ["--max-connections=256"]
        environment:
          MYSQL_DATABASE: ${DB_DATABASE}
          MYSQL_USER: ${DB_USERNAME}
          MYSQL_PASSWORD: ${DB_PASSWORD}
          MYSQL_ROOT_PASSWORD: ${DB_ROOT_PASSWORD}
        volumes:
          - mysql-data:/var/lib/mysql
        healthcheck:
          test: ["CMD", "mysqladmin", "ping", "-h", "localhost"]
          interval: 5s
          timeout: 5s
          retries: 10

    volumes:
      benchkit-runs:
      mysql-data:
    ```
    ::::
  :::

  :::tabs-item{label="MariaDB" icon="i-simple-icons-mariadb"}
    ::::code-tree{defaultValue=".env"}
    ```ini [.env]
    # BenchKit reads these as Laravel's database config.
    # DB_HOST is the service name below, not localhost.
    DB_CONNECTION=mariadb
    DB_HOST=mariadb
    DB_PORT=3306
    DB_DATABASE=benchkit
    DB_USERNAME=benchkit
    DB_PASSWORD=benchkit_secret

    # Only the database container uses this one.
    DB_ROOT_PASSWORD=root_secret
    ```

    ```yaml [docker-compose.yml]
    services:
      benchkit:
        image: serversideup/benchkit-laravel:latest
        ports:
          - "80:8080"
        environment:
          DB_CONNECTION: ${DB_CONNECTION}
          DB_HOST: ${DB_HOST}
          DB_PORT: ${DB_PORT}
          DB_DATABASE: ${DB_DATABASE}
          DB_USERNAME: ${DB_USERNAME}
          DB_PASSWORD: ${DB_PASSWORD}
        volumes:
          - benchkit-runs:/var/www/html/storage/app/runs
        depends_on:
          mariadb:
            condition: service_healthy

      mariadb:
        image: mariadb:11.4
        # Must stay above the app's PHP worker count: each worker holds its own
        # connection, and the default of 151 is below what a large host runs.
        command: ["--max-connections=256"]
        environment:
          MARIADB_DATABASE: ${DB_DATABASE}
          MARIADB_USER: ${DB_USERNAME}
          MARIADB_PASSWORD: ${DB_PASSWORD}
          MARIADB_ROOT_PASSWORD: ${DB_ROOT_PASSWORD}
        volumes:
          - mariadb-data:/var/lib/mysql
        healthcheck:
          test: ["CMD", "healthcheck.sh", "--connect", "--innodb_initialized"]
          interval: 5s
          timeout: 5s
          retries: 10

    volumes:
      benchkit-runs:
      mariadb-data:
    ```
    ::::
  :::

  :::tabs-item{label="PostgreSQL" icon="i-simple-icons-postgresql"}
    ::::code-tree{defaultValue=".env"}
    ```ini [.env]
    # BenchKit reads these as Laravel's database config.
    # DB_HOST is the service name below, not localhost.
    DB_CONNECTION=pgsql
    DB_HOST=postgres
    DB_PORT=5432
    DB_DATABASE=benchkit
    DB_USERNAME=benchkit
    DB_PASSWORD=benchkit_secret
    ```

    ```yaml [docker-compose.yml]
    services:
      benchkit:
        image: serversideup/benchkit-laravel:latest
        ports:
          - "80:8080"
        environment:
          DB_CONNECTION: ${DB_CONNECTION}
          DB_HOST: ${DB_HOST}
          DB_PORT: ${DB_PORT}
          DB_DATABASE: ${DB_DATABASE}
          DB_USERNAME: ${DB_USERNAME}
          DB_PASSWORD: ${DB_PASSWORD}
        volumes:
          - benchkit-runs:/var/www/html/storage/app/runs
        depends_on:
          postgres:
            condition: service_healthy

      postgres:
        image: postgres:17
        # Must stay above the app's PHP worker count: each worker holds its own
        # connection, and the default of 100 is below what a large host runs.
        command: ["postgres", "-c", "max_connections=256"]
        environment:
          POSTGRES_DB: ${DB_DATABASE}
          POSTGRES_USER: ${DB_USERNAME}
          POSTGRES_PASSWORD: ${DB_PASSWORD}
        volumes:
          - postgres-data:/var/lib/postgresql/data
        healthcheck:
          test: ["CMD-SHELL", "pg_isready -U ${DB_USERNAME} -d ${DB_DATABASE}"]
          interval: 5s
          timeout: 5s
          retries: 10

    volumes:
      benchkit-runs:
      postgres-data:
    ```
    ::::
  :::
::

Then bring both containers up:

```bash [Terminal]
docker compose up -d
```

::note
`DB_HOST` is the service name from the compose file rather than `localhost`. Containers reach each other by service name on the Compose network, and `localhost` inside the BenchKit container means the BenchKit container itself.
::

The healthcheck on `depends_on` is what keeps your first run honest. Database images accept connections shortly before they are ready to serve them well, and a benchmark that starts in that window measures the warmup rather than the database.

## Why the connection limit is raised

Every database image ships a connection limit meant for ordinary traffic, and a load test is not ordinary traffic. Laravel opens one connection per PHP worker and keeps it, so the database needs more connections than the app has workers — on a host running a few hundred of them, the stock limit is reached partway through the sweep and the DB route starts failing while it is still gaining throughput.

The figure above covers the concurrency BenchKit offers. If your results page reports that the DB route stopped answering at some number of connections, that is this limit: raise it past the worker count shown in the Environment panel and run again.

On Postgres each connection is a separate process, so a very high limit costs memory. Size it to your worker count rather than reaching for the largest number that works.

## On a host without Docker

If you are [running from source](/docs/getting-started/running-from-source), there is no compose file involved. Set the same `DB_*` variables however your platform manages environment variables, and point them at a database it can reach.

## There is no migration step

BenchKit creates the tables it benchmarks when it needs them, and drops them afterwards. You do not run `artisan migrate` and there is no seeder to run. Point BenchKit at a database it can reach, and the database route and the PHP stage use it on the next run.

::note
Your run history is stored as files rather than in the database. Changing the database engine only changes what the benchmark measures. Your saved runs are unaffected.
::

::tip
The environment panel on a result records which database engine produced it. A MySQL run and a SQLite run are never confused with each other when you compare them later.
::
