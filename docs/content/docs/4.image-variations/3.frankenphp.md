...
**PHP-FPM's pool size caps concurrency.** Under FPM a request occupies a worker for its whole duration, including time spent waiting. On the simulated I/O route, the default pool of 20 workers cannot exceed a fixed ceiling however fast the hardware is. If your connection count is above `pm.max_children`, you are measuring the pool rather than the framework. BenchKit detects the pool size and flags the run when this happens.

**Neither side is tuned by default.** Compare tuned against tuned, and say which tuning you used.

::warning
Before you publish a FrankenPHP-versus-FPM comparison, look at the `/bench/io` curve on the FPM side. If it flattens early, the worker pool is what you measured rather than the server — see [Common tuning strategies](/docs/configuration/common-tuning-strategies#worker-mode-frankenphp-and-octane).
::
