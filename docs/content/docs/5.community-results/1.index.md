...
Submitting is optional. BenchKit never sends your results anywhere on its own, and a run only leaves your machine when you press the button.

::note
Three things do leave the server during a run, none of them a result: the network stage talks to Cloudflare, the hardware stage uploads Geekbench's own result to browser.geekbench.com to obtain the score, and after the network stage BenchKit asks RIPE which organization holds the observed ASN, to pre-fill the provider field.
::
...
| The external generator's IP and hostname | The machine driving the load is one you control — often a laptop |
...
The list above applies to what is submitted. The run saved on your own server, and the zip you can download from it, is the full record and still holds your `APP_URL`, the load test target, and the raw YABS output.
...
Gallery cards lead with the `/bench/json` route's throughput, then `/bench/static`, then `/bench/db-read` when a run lacks the others — the same order the app and a shared image use. [Reading your results](/docs/benchmarks/reading-your-results#comparing-two-runs-fairly) explains the choice.

Self-tested and externally-driven runs are two different measurements, so the gallery keeps them apart: self-tests are labeled, shown with a warning, and listed in their own partition. A run where the load generator itself was the bottleneck is rejected outright, because its throughput describes the network path between two machines rather than the server.
